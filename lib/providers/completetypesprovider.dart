...
class completetypesprovider extends ChangeNotifier{
...
Map types={};
 ...
 resetmaterialsnamedata(){
    ...
    notifyListeners();}
 reportsdata()async{
  reports=await typesservice.getdata(sectionname: "complete types");
  notifyListeners();
 }
 editreportsdata(String data,)async{
  editreports=await edittypesservice.getdata(data,"edit complete types");
  notifyListeners();
 }
 typesnamesdata({String ?factory_id})async{
  typesname=[];
  types=await typesservice.gettypesnamesdata(sectionname: "complete types");
